...
console.log(formattedDate)
      ...
      const cancelledShift = await Shift.findOne({
        dateBooking: formattedDate,
      ...
      if (cancelledShift) {
        ...
        const update = await Shift.updateOne(
          { _id: req.body.idShift },
          {
            dateBooking: formattedDate,
            dateShift: formDate,
...
console.log(formattedDate)
  ...
  cancelShift: async function (req, res) {
    try {
      const cancelled = await Shift.updateOne(
        { _id: req.body.idShift },
        {
          statusHour: "cancelled",
        }
      );
      res.status(201).send("Turno Cancelado");
    } catch (error) {
